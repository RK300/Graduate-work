...
namespace techSupport.edit_form
{
    public partial class country_form : Form
    {
        ...
        {
            string query = "SELECT id, name AS [Название] FROM Country";
        ...
        {
            country_edit F2 = new country_edit();
            F2.Text = "Редактирование страны";
        ...
        private void rjButton3_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Вы действительно хотите удалить запись?", "Подтверждение удаления", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
                    SqlCommand command = new SqlCommand($"DELETE FROM Country WHERE id = {dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value}", sqlConnection);
        ...
        private void rjButton5_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = $"SELECT id, name AS [Название] FROM Country WHERE name LIKE '%{textBox1.Text}%'";
                var connectionString = ConfigurationManager.ConnectionStrings["db"].ConnectionString;
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connectionString))
                {
                    DataTable dataTable = new DataTable();
