...
namespace techSupport.Ticket_system
{
    public partial class file_form : Form
    {
        ...
        {
            string query = "SELECT id AS [Номер], image AS [Изображение], date_upload AS [Дата и время загрузки] FROM ImageFiles";
        ...
        {
            /*file_edit F2 = new file_edit();
            F2.Text = "Редактирование файла";
        ...
        private void rjButton3_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Вы действительно хотите удалить запись?", "Подтверждение удаления", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
                    SqlCommand command = new SqlCommand($"DELETE FROM ImageFiles WHERE id = {dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value}", sqlConnection);
