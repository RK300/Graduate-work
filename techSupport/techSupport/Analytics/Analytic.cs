...
namespace techSupport.Analytics
{
    public partial class Analytic : Form
    {
        ...
        public Analytic(int id)
        {
            ...
            combobox(comboBox4, "SELECT id, (CompanyName + ' | ' + surname + ' ' + name + ' ' + patronymic) AS [FIO] FROM [Clients]", "FIO", "id");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (comboBox4.SelectedIndex == -1)
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    string TicketCount = "Всего тикетов: " + CountRows.ToString();
                    string RowIndex = Count.ToString();
                    string Rows = "I" + RowIndex.ToString() + ":K" + RowIndex.ToString();

                    worksheet.Cell("I" + RowIndex).Value = TicketCount;
                    worksheet.Cell("I" + RowIndex).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                    worksheet.Cell("I" + RowIndex).Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
                    worksheet.Cell("I" + RowIndex).Style.Font.Bold = true;
                    worksheet.Cell("I" + RowIndex).Style.Font.FontName = "Times New Roman";
                    worksheet.Cell("I" + RowIndex).Style.Font.FontSize = 16;
                    worksheet.Range(Rows).Merge();
            ...
            else
            {
                ...
                string query = $"SELECT CompanyName FROM Clients WHERE id = {comboBox4.SelectedValue.ToString()}";
                var connectionString = ConfigurationManager.ConnectionStrings["db"].ConnectionString;
                using (SqlDataAdapter adapter = new SqlDataAdapter(query, connectionString))
                {
                    DataTable dataTable = new DataTable();
                    adapter.Fill(dataTable);
                    name = dataTable.Rows[0][0].ToString();
                }

                using (var workbook = new XLWorkbook())
                {
                    var worksheet = workbook.Worksheets.Add("Тикеты за период");
                    string filename = "Тикеты от " + name + " за период (" + dateTimePicker1.Value.ToString("D") + " - " + dateTimePicker2.Value.ToString("D") + ").xlsx";
                    ...
                    worksheet.Cell("A2").Value = "Тикеты от " + name + " за период";
                    worksheet.Cell("A2").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                    worksheet.Cell("A2").Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
                    worksheet.Cell("A2").Style.Font.Bold = true;
                    worksheet.Cell("A2").Style.Font.FontName = "Times New Roman";
                    ...
                    int Count = 0;
                    int CountRows = 0;
                    string query2 = $"SELECT Clients.CompanyName AS [Клиент], Products.name AS [Продукт], (Worker.surname + ' ' + Worker.name + ' ' + Worker.patronymic) AS [Принявший сотрудник], Type.name AS [Тип проблемы], Ticket.header AS [Заголовок], Ticket.description AS [Описание], Ticket.priority AS [Приоритет], Ticket.status AS [Статус], Ticket.application_data AS [Дата подачи], Ticket.completion_data AS [Дата закрытия], Ticket.actual_data AS [Фактическая дата закрытия] FROM Ticket, Clients, Products, Worker, Type WHERE Ticket.client = Clients.id AND Ticket.product = Products.id AND Ticket.worker = Worker.id AND Ticket.type = Type.id AND Ticket.application_data >= CAST('{dateTimePicker1.Value.ToString()}' AS DATETIME) AND Ticket.application_data <= CAST('{dateTimePicker2.Value.ToString()}' AS DATETIME) AND Clients.id = {comboBox4.SelectedValue.ToString()} ORDER BY Ticket.application_data ASC";
